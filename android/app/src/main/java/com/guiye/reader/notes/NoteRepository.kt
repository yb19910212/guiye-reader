...
data class ReadingNote(val id: String, val bookId: String, val bookTitle: String, val text: String, val locator: String?, val createdAt: Long)
...
class NoteRepository(context: Context) {
    ...
    fun all(): List<ReadingNote> {
        ...
        return (0 until array.length()).mapNotNull { index ->
            runCatching { array.getJSONObject(index) }.getOrNull()?.let { json ->
                ReadingNote(json.getString("id"), json.getString("bookId"), json.getString("bookTitle"), json.getString("text"), json.optString("locator").ifBlank { null }, json.getLong("createdAt"))
            }
    ...
    private fun save(notes: List<ReadingNote>) {
        ...
        notes.forEach { note -> array.put(JSONObject().apply {
            put("id", note.id); put("bookId", note.bookId); put("bookTitle", note.bookTitle); put("text", note.text); put("locator", note.locator); put("createdAt", note.createdAt)
        }) }
